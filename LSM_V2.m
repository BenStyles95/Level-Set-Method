...
% ---------------------------------------------------------------------- %
% PHI FUNCTION --------------------------------------------------------- %
P = ((X + 25).^2 + (Y + 25).^2).^(0.5) - 5;         % Circle
P = min(P,((X - 25).^2 + (Y - 25).^2).^(0.5) - 5);  % 2nd circle 
% ---------------------------------------------------------------------- %
...
for i = 1:S
    ...
    light('Position',[0 0 max(G(:))+100],'Style','local'); % Position local light
    pbaspect([1 1 1])
    grid on
    hold on
